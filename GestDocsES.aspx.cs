...
namespace LabMetro
{
    ...
    /// </summary>
    public partial class GestDocsES : System.Web.UI.Page
    {
        ...
        private string MSG_PDF_INEXISTENTE = "Crear PDF urgente.";
        private string MSG_PDF_CRIARNOVO = "Crear PDF.";
        private string MSG_PDF_CORROMPIDO = "PDF corrompido. Crear PDF.";
        ...
        //TEMPLATES
        private static string templatePagina = (string)ConfigurationManager.AppSettings["TEMPLATE_PAGINA_ES"]; //fundo branco - SO HA ESTE
            ...
            else
            {
                ...
                else
                {
                    if (!Page.IsPostBack)
                    {
                        ...
                        //NENHUM OUTRO DOCUMENTO COM O MESMO NOME. 
                        ...
                        //idEstadoCertificado=(1,2,5,6) n.existente, calibra.s/val,rejeit.tecn, rejeit.resp. //podem tb vir docs em estado 3                               por causa dos OR's na query
        ...
        private string MergePDFInicial()
        {
            ...
            //por isso tenho de pensar numa maneira de comparar cada idServico com todos os seus certificados associados apenas uma vez
            {
                ...
                else
                {
                    ...
                    if (drv.Row["nomeDocumento"].ToString() != "")  // A NAO CONFUNDIR COM nomeFichCalibracao. 
                    {

                        FileInfo[] fileInfosPastaOrigem = dirInfoPastaOrigem.GetFiles("*" + drv.Row["nomeDocumento"].ToString() + "*");

                        for (int i = 0; i < fileInfosPastaOrigem.GetLength(0); i++) 
                        {
                            ...
                            if (drv.Row["idEstadoCertificado"].ToString() == "3")//já tem certificado nao posso carregar outro, mas tarde vou ter de apagar a datarow
                            {
                                ...
                                //se o documento já foi criado nao pode ser criado mais vez nenhuma a não ser que esteja no estado 5 ou 6 
                            //e para o tipo de certificado 1C tb no estado 1 e isto pode ser validado logo antes de mandar os ficheiros para o outro lado. 
                            ...
                            else
                            {
                                ...
                                string caminhoSimboloAcreditacao = pastaSimbolos + "\\" + strSimboloAcreditacao + ".jpg";
                                string caminhoSimboloAcreditacaoHome = pastaSimbolos + "\\" + strSimboloAcreditacao + "_home.jpg";


                                //TEMPLATE É AGORA SEMPRE IGUAL
                                //se existe simbolo de acreditacao, junta o simbolo
                                if (strSimboloAcreditacao != "")
                                {
                                    errMessage += mergeFiles(caminhoDocumentoDestino, caminhoDocumentoOrigem, caminhoTemplate, caminhoSimboloAcreditacao,caminhoSimboloAcreditacaoHome, caminhoDocumentoBackup, fileInfosPastaOrigem[i], false);//false=bAlteracaoTemplate
                                }

                                else  //manda um null no simbolo 
                                {
                                    errMessage += mergeFiles(caminhoDocumentoDestino, caminhoDocumentoOrigem, caminhoTemplate, null,null, caminhoDocumentoBackup, fileInfosPastaOrigem[i], false);//false=bAlteracaoTemplate
                                }
                                
                                
                                if (errMessage.IndexOf(MSG_PDF_CORROMPIDO) > -1)
                                {
        ...
        private string mergeFiles(string caminhoDocumentoDestino, string caminhoDocumentoOrigem, string caminhoTemplate, string caminhoSimboloAcreditacao, string caminhoSimboloAcreditacaoHome, string caminhoDocumentoBackup, FileInfo fileOriginal, bool bAlteracaoTemplate)
        {
            ...
            try
            {
                ...
                iTextSharp.text.Image imgSimboloAcreditacaoHome;
                ...
                if (caminhoSimboloAcreditacao != null)
                {
                    ...
                    {
                        errMessage += fileOriginal.ToString() + " Problema com o simbolo. Verifique a nomenclatura do simbolo na página de gestão de tipos de equipamento (falar com Resp.Técn).<br />";
                        return errMessage;
                ...
                //para ver se encontra a img do simbolo e dar mensagem se ha alguma problema. 
                if (caminhoSimboloAcreditacaoHome != null)
                {
                    try
                    {
                        imgSimboloAcreditacaoHome = iTextSharp.text.Image.GetInstance(caminhoSimboloAcreditacaoHome);
                    }
                    catch
                    {
                        errMessage += fileOriginal.ToString() + " Problema com o simbolo da pagina inicial. Verifique a nomenclatura do simbolo na página de gestão de tipos de equipamento (falar com Resp.Técn).<br />";
                        return errMessage;
                ...
                {	//***************************************************************************************************
                    //***************************************************************************************************
                    //para todas as extensoes 1C, 1R, 2R, 1A etc. não pode ser validado um novo documento se já existe um certificado com esse tipo. 

                    //tb não podem entrar novos certifs que estejam à espera de serem validados pelo RT Se ainda não existe a extensão, 
                    //podem entrar todos menos os que estão no estado TRES
                    ...
                    if (letraSigla.ToUpper() == "A")
                    {
                        ...
                        {
                            throw new Exception(" - O aditamento já existe. (" + refPai + "-" + siglaIn + ")");  
                ...
                while (j < readerRelatorio.NumberOfPages)
                {
                    ...
                    //se tem simbolo: SIMBOLO GRANDE NA 1A PAGINA à esquerda E SIMBOLO MAIS PEQUENO NAS RESTANTES, à direita
                    if ((caminhoSimboloAcreditacao != null) && (caminhoSimboloAcreditacaoHome != null))
                    {

                        
                        imgSimboloAcreditacaoHome = iTextSharp.text.Image.GetInstance(caminhoSimboloAcreditacaoHome); //imagem maior na primeira pagina
                        imgSimboloAcreditacao = iTextSharp.text.Image.GetInstance(caminhoSimboloAcreditacao); //imagem mais pequena - normal
                        

                        if (j == 1) //primeira pagina simbolo grande do lado esquerod (na outra aplicacao aqui era == 0 ???
                        {

                            //para saber as medidas tenho de converer pixels to points de acordo com o dpi da imagem
                            // neste caso 150 dpi: points = (pixel / dpi) * 72 
                            cb.AddImage(imgSimboloAcreditacaoHome, 114.72f, 0, 0, 87.36f, 30, 760); //mudar posicao
                            //http://stackoverflow.com/questions/4591964/how-do-i-add-images-to-a-pdf-with-itextsharp
                        ...
                        else //restantes paginas: simbolo
                        {
                            cb.AddImage(imgSimboloAcreditacao, 45.12f, 0, 0, 36f, 520, 760); //mudar posicao
                        }
        ...
        protected void cbTemplate_checked(object sender, System.EventArgs e)
        {
            ...
            for (int i = 0; i < fi.GetLength(0); i++)
            {
                ...
                else
                {
                    ...
                    string strSimboloAcreditacao = DT.Rows[index]["simbolo"].ToString().TrimEnd();
                    ...
                    if (cb.Checked) //se está checked, tem simbolo
                    {
                        ...
                        else//se não é uma reparação
                        {
                            ...
                            lblMessage.Text += mergeFiles(caminhoDestino, caminhoOrigem, caminhoTemplate,caminhoSimboloAcreditacao,caminhoSimboloAcreditacaoHome,  caminhoBackup, fi[i], true);//true=bAlteracaoTemplate
                    ...
                    else //se não está checked quero mandar para as templates sem simbolo  
                    {
                        ...
                        {
                                lblMessage.Text += mergeFiles(caminhoDestino, caminhoOrigem, caminhoTemplate, null,null, caminhoBackup, fi[i], true);//true=bAlteracaoTemplate
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    lblMessage.Text = "Não escolheu nenhum documento.";
                }
            }
        }
        #endregion
